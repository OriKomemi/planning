...
def get_safe_log_filename():
    ...
    return f'formula_{timestamp}.log'

def init_logger(output_dir="output/logs"):
    global OUTPUT_DIR
    ...
    logging_config = {
        ...
        'handlers': {
            ...
            'file_handler': {
                ...
                'level': 'DEBUG',
                'formatter': 'detailed',
                'filename': os.path.join(OUTPUT_DIR, get_safe_log_filename()),
            ...
            'error_file_handler': {
                ...
                'level': 'ERROR',
                'formatter': 'detailed',
                'filename': os.path.join(OUTPUT_DIR, f'formula_errors_{time.strftime("%Y-%m-%d_%H-%M-%S")}.log'),
                'maxBytes': 10485760,  # 10MB
                'backupCount': 5
            },
            'performance_handler': {
                ...
                'level': 'INFO',
                'formatter': 'detailed',
                'filename': os.path.join(OUTPUT_DIR, f'formula_performance_{time.strftime("%Y-%m-%d_%H-%M-%S") }.log'),
                'maxBytes': 10485760,  # 10MB
                'backupCount': 3
            }
        },
