...
class Manager:
    def __init__(self, providers, nodes, dt, enable_plots, output_dir, simulation=False, exporter=None, recorder=None):
        ...
        self.first_orange_signal = True
        self.first_orange_time = None
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    for p in self.providers:
                        ...
                        if provider_data is None:
                            log.warning(f"Provider {p.__class__.__name__} returned None")
                            continue
                        log.info(f"Read from {p.__class__.__name__}: "f"{len(provider_data) if provider_data else 0} items, read time: {time.time() - start_lidar:.4f}s")
                ...
                if data["orange_signal"] == True:
                    if self.first_orange_signal:
                        log.warning("First orange signal detected")
                        self.first_orange_signal = False
                        self.first_orange_time = time.time()
                    elif not self.first_orange_signal and time.time() - self.first_orange_time > 30:
                        log.warning("Subsequent orange signal detected, stopping control loop")
                ...
                if self.enable_plots:
                    ...
                    if self.simulation and self.laps_limit(data.get("referee_state"), max_laps=20):
                            self._generate_final_plots()
                            self._cleanup()
                            break
                if self.exporter:
            ...
            self._generate_final_plots()
            self.recorder.close() if self.recorder else None
        except Exception as e:
    ...
    def laps_limit(self, referee_state, max_laps: int = 1):
        """Limit the number of laps for simulation purposes."""   
        limit_reached = len(referee_state.laps ) >= max_laps
        if limit_reached:
            log.info(f"Maximum laps reached: {len(referee_state.laps)} >= {max_laps}")
            log.info("Stopping simulation")
            log.info(f"referee_state.laps: {referee_state.laps}")
    ...
    def _generate_final_plots(self):
        ...
        self.plotter.export_to_video(f"{self.output_dir}/video.mp4")
        try:
            ...
            if self.plot_data.states is not None and self.plot_data.cx is not None and self.plot_data.cy is not None:
                ...
                #plot acceleration
                self.plotter.plot_acceleration(self.plot_data.states)
