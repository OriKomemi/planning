...
_FIGS: List[Figure] = []
_LIVE = False #doesnt do shit as of right now.
...
class PlotManager:
    ...
    def plot_car(x, y, yaw, steer=0.0, truckcolor="-k"):
        ...
        TREAD = 0.7  # [m]
        WB = 2.5  # [m]
        # conver steer from degrees to radians
    ...
    @staticmethod
    def plot_cones(cones_by_type, cones_lidar=[],key=None):
        """
        Plot cones based on their type (left or right).
        """
    ...
    def draw_frame(
        ...
        if cones_by_type is not None and cones_lidar is not None:
        ...
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR).copy()
        # add logic of rotate/flip
    ...
    def plot_path_deviation1(cx, cy, states, X,Y,cones_by_type,cones_lidar):
        ...
        # cones_by_type = self.plot_data.cones_map
# If X (and similarly Y) is a list of 1D arrays, flatten it:
        ...
        plt.title(f"Path Deviation (MSE: {mse:.2f}, RMSE: {rmse:.2f})")
    ...
    def plot_control_inputs(states, dt=conf.dt):
        ...
        plt.plot(time, states.acceleration[:min_length], label="Acceleration command", color='orange')
    ...
    def plot_acceleration(states, dt=conf.dt):
        ...
        # print("Available attributes:", dir(states))  # Debug line
    
    # If states has direct acceleration arrays (like in plot_gg)
        if hasattr(states, 'a_longitudinal') and hasattr(states, 'a_lateral'):
            # Use existing acceleration data
            longitudinal_accel = np.array(states.a_longitudinal)
            lateral_accel = np.array(states.a_lateral)
            
            # Calculate total acceleration magnitude
            total_accel = np.sqrt(longitudinal_accel**2 + lateral_accel**2)
            
            min_length = len(total_accel)
            time = np.linspace(0, (min_length - 1) * dt, min_length)

            fig = _auto_fig()
            plt.figure(fig.number)
            plt.plot(time, longitudinal_accel, label="Longitudinal Acceleration [m/s²]", color='green')
            plt.plot(time, lateral_accel, label="Lateral Acceleration [m/s²]", color='orange')
            plt.plot(time, total_accel, label="Acceleration magnitude [m/s²]", color='red')
            ...
            print("No acceleration data available in states object")
    @staticmethod
    ...
    def plot_all_accelerations(states, dt=conf.dt):
        ...
        # Try different possible attribute names for Vector3r
        angular_velocity = np.array([v.z_val for v in states.v_angular])
    ...
    def export_to_video(self, output_path, dpi=150, fps=10):
        """
